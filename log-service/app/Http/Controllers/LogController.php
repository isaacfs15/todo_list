 ...
 * LogController.php
 *
 * Responsável por receber, listar e filtrar registros de log
 * enviados pelo task-service (Node.js) via HTTP.
 *
 * CONTRATO DO PAYLOAD RECEBIDO (deve coincidir com servicoLog.js):
 *   acao      --- string: CRIACAO | CONCLUSAO | REABERTURA | ERRO
 *   usuarioId --- integer (camelCase, conforme enviado pelo JS)
 *   tarefaId  --- integer|null
 *   descricao --- string
 *
 * O controller recebe camelCase e grava em snake_case no banco.
 *
 * Rotas atendidas:
 *   POST /api/logs              → store()
 *   GET  /api/logs              → index()
 *   GET  /api/logs/{usuarioId}  → porUsuario()
 */
